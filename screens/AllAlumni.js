...
import { View, Text, Button, StyleSheet, FlatList, ActivityIndicator,Switch,RefreshControl,StatusBar} from 'react-native';
import {
  ContainerLst,
  Card,
  UserInfo,
...
const AllAlumni=({route,navigation})=>{
 ...
 const url = `http://${mp}/FypApi/api/Student/GetStudentsOnly?sid=${foo}`;
 fetch(url,{
     method:'GET',
 }).then((response)=>response.json())
 .then((response)=>{
